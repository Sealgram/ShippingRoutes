...
def theory(locations):
    ...
    for x in locations:
        for ship in x:
            #makes sure no ships duplicate and sit on top of each other, but this is covered by the adjacency constraint and the 1 ship per time constraint 
            #constraint.add_implies_all(E, ship, ~Ship(ship.time+1, ship.x, ship.y, ship.has_cargo_type) & ~Ship(ship.time-1, ship.x, ship.y, ship.has_cargo_type))
            if ship.time != 0 and ship.time < time:
                print("ran first")
                constraint.add_implies_all(E, ship, (Ship(ship.time+1, ship.x+1, ship.y, ship.has_cargo_type) | Ship(ship.time+1, ship.x-1, ship.y, ship.has_cargo_type) | Ship(ship.time+1, ship.x, ship.y+1, ship.has_cargo_type) | Ship(ship.time+1, ship.x, ship.y-1, ship.has_cargo_type) & 
                (Ship(ship.time-1, ship.x+1, ship.y, ship.has_cargo_type) | Ship(ship.time-1, ship.x-1, ship.y, ship.has_cargo_type) | Ship(ship.time-1, ship.x, ship.y+1, ship.has_cargo_type) | Ship(ship.time-1, ship.x, ship.y-1, ship.has_cargo_type))))
                #constraint.add_exactly_one(E, Ship(ship.time+1, ship.x+1, ship.y, ship.has_cargo_type), Ship(ship.time+1, ship.x-1, ship.y, ship.has_cargo_type), Ship(ship.time+1, ship.x, ship.y+1, ship.has_cargo_type), Ship(ship.time+1, ship.x, ship.y-1, ship.has_cargo_type))
                #constraint.add_exactly_one(E, Ship(ship.time-1, ship.x+1, ship.y, ship.has_cargo_type), Ship(ship.time-1, ship.x-1, ship.y, ship.has_cargo_type), Ship(ship.time-1, ship.x, ship.y+1, ship.has_cargo_type), Ship(ship.time-1, ship.x, ship.y-1, ship.has_cargo_type))
            elif ship.time == 0:
                print("ran second")
                #constraint.add_exactly_one(E, Ship(ship.time+1, ship.x+1, ship.y, ship.has_cargo_type), Ship(ship.time+1, ship.x-1, ship.y, ship.has_cargo_type), Ship(ship.time+1, ship.x, ship.y+1, ship.has_cargo_type), Ship(ship.time+1, ship.x, ship.y-1, ship.has_cargo_type))
